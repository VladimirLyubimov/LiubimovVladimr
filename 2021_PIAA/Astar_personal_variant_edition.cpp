...
public:
    ...
    void calcEveristic(Node target_node) {
        m_everistic_val = abs(static_cast<float>(m_val) - static_cast<float>(target_node.getValue()));
    }
...
void makePath(std::vector<Node>& nodes, std::vector<int>& path, int finish) {
...
std::vector<int> Astar(Graph graph, int start, int finish, int**& Atable) {
...
int main() {
    ...
    std::vector<int> path = Astar(graph, start, finish, Atable);
